...
apply_payload "packages/sdk/src/types.ts" "sdk_blog_opts@v8.4.2r2" "export interface BlogOpts {" "sdk_blog_opts@v8.4.2r2.pl" "replace_block" "}"
...
apply_payload "packages/cli/src/config.ts" "config_patch_ws@v8.4.2r2" "  blog?: {" "config_patch_ws@v8.4.2r2.pl" "replace_line"

# 6. ORCHESTRATOR BROADCAST AUTHORITY (CR-006.v1 Refinement)
apply_payload "packages/cli/src/pulse.ts" "pulse_config_imports@v8.4.2r2" "import { board," "pulse_config_imports@v8.4.2r2.pl" "replace_line"
apply_payload "packages/cli/src/pulse.ts" "blog_authority_itb@v8.4.2r2" 'const [cmd, ...args] = process.argv.slice(2);' "blog_authority_itb@v8.4.2r2.pl" "replace_line"

# Syntax Clean
BLOCK_START='case "blog": {'
BLOCK_END='    break;
  }'
apply_payload "packages/cli/src/pulse.ts" "blog_syntax_itb@v8.4.2r2" "$BLOCK_START" "blog_syntax_itb@v8.4.2r2.pl" "replace_block" "$BLOCK_END"
...
apply_payload "packages/cli/src/commands/blog.ts" "blog_target_itb@v8.4.2r2" "$TARGET_START" "blog_target_itb@v8.4.2r2.pl" "replace_block" "$TARGET_END"
