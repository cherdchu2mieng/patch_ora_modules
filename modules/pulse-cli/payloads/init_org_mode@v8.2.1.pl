...
export async function init() {
  ...
  try {
    ...
    if (!isOrg) {
      // --- [U]ser Mode (STABLE: NO GATEWAY PROMPT) ---
      orchestrator = (await ask(rl, "Orchestrator Oracle (e.g. gemi): ")).trim();
    } else {
      // --- [O]rg Mode (REQUIRED GATEWAY) ---
    ...
    const config: any = {
      ...
      projectNumber,
      oracleRepos,
      orchestrator: isOrg ? undefined : (orchestrator || undefined),
      gateway: isOrg ? undefined : (gateway || undefined),
      routing
    };
    ...
    if (isOrg && gateway) {
      ...
      if (userConfigPath) {
        ...
        console.log(`✓ Gateway synchronized successfully.`);
      }
    }
