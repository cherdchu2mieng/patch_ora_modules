...
export async function chb(masterItemIndex: number, opts: { delegated?: boolean } = {}) {
  try {
    ...
    if (!isITB) {
      console.error("❌ Error: 'chb' delegation currently only supported from ITB board.");
      return;
    }
    ...
    // 3. Status Check (Strictly 'Assigned')
    const currentStatus = (item.status || "").toLowerCase();
    if (currentStatus !== "assigned") {
      console.error(`❌ Error: Task must be in 'Assigned' status to initiate delegation (Current: ${item.status || 'unknown'}).`);
      return;
    }

    // 4. Target Resolution (AIB)
    const resolvedTarget = ctx.board?.AIB;
    if (!resolvedTarget) {
      console.error("❌ Error: No target AIB repository found in config.");
      return;
    }

    if (opts.delegated) {
      console.log(`📡 Initiating Task Delegation (ITB -> AIB): ${resolvedTarget}...`);

      // 5. Remote Issue Creation
      ...
      console.log("💡 Use 'pulse chb <index> --Delegated' to initiate delegation.");
    }
