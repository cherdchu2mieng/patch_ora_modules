#!/bin/bash
# Pulse Patch v7.8 - THE ULTIMATE CUMULATIVE PATCH
# Includes features from v7.5, v7.6, v7.7, and v7.8
...
echo "Patching pulse-cli at $PULSE_PATH to v7.8..."
...
SDK_TYPES_EOF
echo "Patched packages/sdk/src/types.ts"
...
if 'export async function setFieldOnItem' not in content:
    ...
    open(path, 'w').write(content)
SDK_GH_EOF
echo "Patched packages/sdk/src/github.ts"
...
if 'orchestrator: cfg.orchestrator' not in content:
    content = content.replace('return { org: cfg.org, projectNumber: cfg.projectNumber, gateway: cfg.gateway };', 
                         'return { org: cfg.org, projectNumber: cfg.projectNumber, gateway: cfg.gateway, orchestrator: cfg.orchestrator };')
open(path, 'w').write(content)
CLI_CFG_EOF
echo "Patched packages/cli/src/config.ts"

# 4. CLI init.ts (COMPREHENSIVE)
...
# Full Replacement of the init function
init_impl = r'''export async function init() {
  ...
  try {
    ...
    if (!fs.existsSync(targetPath)) {
      ...
      let changed = false;
      if (gateway) { config.gateway = gateway; changed = true; }
      if (orchestrator) { config.orchestrator = orchestrator; changed = true; }
      if (changed) {
        fs.writeFileSync(targetPath, JSON.stringify(config, null, 2) + '\n');
...
open(path, 'w').write(content)
CLI_INIT_EOF
echo "Patched packages/cli/src/commands/init.ts"
...
open(path, 'w').write(content)
CLI_ADD_EOF
echo "Patched packages/cli/src/commands/add.ts"
...
if "if (ctx.gateway) {" not in content:
    ...
    try {
      ...
      const aiTeamItems = gatewayItems.filter(i => (i as any).client === "AI-TEAM");
      ...
      console.log(`  Gateway sync error: ${e.message}`);
    }
  }'''
...
open(path, 'w').write(content)
CLI_SCAN_EOF
echo "Patched packages/cli/src/commands/scan.ts"
...
# Surgical replace for cases
content = content.replace('case "set":\n  case "s":', 'case "set":\n  case "s":\n    enforceAuth();')
content = content.replace('case "triage":\n  case "tr":', 'case "triage":\n  case "tr":\n    enforceAuth();')

# Replace the add case block using regex to be safe against previous corruptions
# but on a clean file it should be straightforward
content = re.sub(r'  case "add":\n  case "a": \{[\s\S]+?break;\n  \}', add_impl, content)

open(path, 'w').write(content)
CLI_PULSE_EOF
echo "Patched packages/cli/src/pulse.ts"

echo "v7.8 Patch Applied successfully."
