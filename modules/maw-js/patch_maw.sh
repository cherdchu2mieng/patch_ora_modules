...
# --- 3.1 Patch src/commands/shared/wake-pane-size.ts ---
python3 - <<'PY_EOF'
import os
path = os.path.join(os.environ['MAW_PATH'], 'src/commands/shared/wake-pane-size.ts')
if os.path.exists(path):
    content = open(path).read()
    if '"window-size", "manual"' in content:
        content = content.replace('"window-size", "manual"', '"window-size", "latest"')
        open(path, 'w').write(content)
        print('✓ Updated wake-pane-size.ts (manual -> latest)')
    else:
        print('i wake-pane-size.ts already patched')
else:
    print('X wake-pane-size.ts not found')
PY_EOF
