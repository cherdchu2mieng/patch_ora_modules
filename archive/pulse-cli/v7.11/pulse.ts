...
function enforceAuth() {
  ...
  if (!ctx.orchestrator) {
    console.error("Orchestrator not found. Please run 'pulse init' to configure authority.");
    process.exit(1);
  }
  const currentOracle = getCurrentOracle();
  if (currentOracle !== ctx.orchestrator) {
    console.error(`Access Denied: Only oracle '${ctx.orchestrator}' can manage fields. (Current: ${currentOracle || 'unknown'})`);
    process.exit(1);
  }
}
...
switch (cmd) {
  ...
  case "add":
  case "a": {
    // [Feature: Org Mode Gateway Support]
    let titleIndex = 0;
  ...
  case "s":
    ...
    // [Feature: Hash Prefix Support (#)]
    const itemNum = args[0].startsWith("#") ? args[0].slice(1) : args[0];
    await set(parseInt(itemNum), ...args.slice(1));
    break;
  case "field-add":
  case "fa":
    if (!args[0] || !args[1]) {
